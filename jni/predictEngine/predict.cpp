...
//default parameters
   ...
   gpuIndex = -1;
   weightsFile = "/data/data/com.sony.openclexample1/app_execdir/weightstface1.dat";//weightstodelete.dat";///weightstface1.dat";//weightstodelete.dat";//weightsTest13july.dat";//"weights.dat";
   batchSize = 128;
   inputFile = "";
   outputFile = "";
   outputLayer = -1;
...
void PredictionModel::go0(ConfigPrediction config) {
	...
	int ignI;
	float ignF;
	WeightsPersister::loadWeights(config.weightsFile, string("netDef=")+netDef, net, &ignI, &ignI, &ignF, &ignI, &ignF);
...
void PredictionModel::go(ConfigPrediction config) {
    ...
    if(config.inputFile == "") {
        ...
        if(verbose){
        	...
        	//cout << "N " << N << " planes " << numPlanes << " size " << imageSize << endl;
    }
...
//    }
    ...
    if(config.weightsFile == "") {
    	LOGE( "weightsFile not specified");
    ...
    if (!WeightsPersister::loadConfigString(config.weightsFile, netDef) ){
    	LOGE( "Cannot load network definition from weightsFile.");
...
//    cout << "net def from weights file: " << netDef << endl;
    ...
    if(!NetdefToNet::createNetFromNetdefPrediction(config.batchSize,net, netDef, weightsInitializer) ) {
        return;
    }
    //LOGE( "je suis la0.5");
 ...
 //LOGE( "je suis la 1.1");
    ...
    while(more) {
        ...
        if(!config.writeLabels) {
        	...
        	//LOGE( "je suis la 1.46");
        	SoftMaxLayerPredict *softMaxLayer = dynamic_cast< SoftMaxLayerPredict *>(net->getLayer(config.outputLayer) );
            if(softMaxLayer == 0) {
            	//LOGE( "must choose softmaxlayer, if want to output labels" );
                //cout << "must choose softmaxlayer, if want to output labels" << endl;
                return;
    ...
    delete[] inputData;
    delete[] labels;
    //LOGE( "je suis la 1.85");
    delete weightsInitializer;
    //LOGE( "je suis la 1.86");
    delete net;
    //LOGE( "je suis la 1.9");
    //delete cl;
}
