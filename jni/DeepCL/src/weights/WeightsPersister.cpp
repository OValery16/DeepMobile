...
STATIC void WeightsPersister::persistWeights(std::string filepath, std::string trainingConfigString, NeuralNet *net, int epoch, int batch, float annealedLearningRate, int numRight, float loss) { // we should probably rename 'weights' to 'model' now that we are storing normalization data too?
    ...
    LOGI( "DeepCL/src/weights/WeightsPersister.cpp: 1");
    FileHelper::writeBinary(filepath + "~", reinterpret_cast<char *>(persistArray), 
        headerLength + totalWeightsSize * sizeof(float));
    LOGI( "DeepCL/src/weights/WeightsPersister.cpp: 2");
    FileHelper::remove(filepath);
    LOGI( "DeepCL/src/weights/WeightsPersister.cpp: 3");
    FileHelper::rename(filepath + "~", filepath);
    LOGI( "DeepCL/src/weights/WeightsPersister.cpp: wrote weights to file, filesize %d KB", (( headerLength + totalWeightsSize) *sizeof(float)/1024));
...
LOGI( "DeepCL/src/weights/WeightsPersister.cpp: exists(filepath) ){");
        ...
        long fileSize;
        char * data = FileHelper::readBinary(filepath, &fileSize);
        LOGI( "DeepCL/src/weights/WeightsPersister.cpp: readBinary");
        if(!checkData(data, headerSize, fileSize) ) {
        	LOGI( "DeepCL/src/weights/WeightsPersister.cpp: checkData1");
            delete [] data;
            return false;
        }
LOGI( "DeepCL/src/weights/WeightsPersister.cpp: checkData2");
        data[headerSize - 1] = 0; // null-terminate the string, if not already done

        // + skip the 'netdef='
        const int *dataAsInts = reinterpret_cast<const int *>(data);
        int version = dataAsInts[1];
        LOGI( "DeepCL/src/weights/WeightsPersister.cpp: version %d",version);
        if(version == 1 || version == 3) {
        	LOGI( "DeepCL/src/weights/WeightsPersister.cpp: configString %s ",data);
        	configString.clear();
            configString.append (data + 7 * 4 + 7);
            LOGI( "DeepCL/src/weights/WeightsPersister.cpp: version %s",configString.c_str());
        } else {
