...
#if DEEPCL_VERBOSE == 1
LOGI( "DeepCL/src/util/JpegHelper.cpp: write");
#endif
...
//    string filename = "foo.jpeg";
    ...
    if((int)cinfo.output_width != width) {
    	LOGE("error reading width %s!=%s",toString(cinfo.output_width).c_str(),toString(width).c_str());
    ...
    if((int)cinfo.output_height != height) {
    	LOGE("error reading height %s!=%s",toString(cinfo.output_height).c_str(),toString(height).c_str());
    ...
    if((int)cinfo.output_components != planes) {
    	LOGE("error reading planes %s!=%s",toString(cinfo.output_components).c_str(),toString(planes).c_str());
